            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        ...
                        <span className={`text-sm font-semibold font-inter ${!workspaceId ? 'text-text-primary' : 'text-[#535862]'}`}>{isAgentLevel ? '...' : 'Enterprise'}</span>
                    </Link>
                        ...
                        <>
                            ...
                            <Link 
                                ...
                                className={`h-8 py-1.5 px-3 rounded-lg flex items-center gap-2 transition-colors cursor-pointer group ${
                                    !iflowId ? 'bg-white border border-[#D0D5DD] shadow-[0_1px_2px_rgba(16,24,40,0.05)] hover:bg-gray-50' : 'border border-transparent hover:bg-white/50'
                                }`}
                            >
                                <Building2 className="w-4 h-4 text-[#FF5714]" />
                                <div className={`text-sm font-semibold font-encode leading-5 flex items-center gap-1 ${!iflowId ? 'text-[#344054]' : 'text-[#535862]'}`}>
                                    <span className='text-[#535862] font-medium text-xs mb-[1px]'>Workspace</span> 
                                    <span className='mt-[0.5px]'>| {displayWorkspaceName}</span>
                                </div>
                                {!iflowId && <ChevronDown className="w-4 h-4 text-[#667085] transition-transform group-hover:translate-y-0.5" />}
                            </Link>
                        </>
                    )}
                        ...
                        <>
                            ...
                            <Link 
                                ...
                                className={`h-8 py-1.5 px-3 rounded-lg flex items-center gap-2 cursor-pointer group transition-all ${
                                    !agentId ? 'bg-white border border-[#D0D5DD] shadow-[0_1px_2px_rgba(16,24,40,0.05)] hover:bg-gray-50' : 'border border-transparent hover:bg-white/50'
                                }`}
                            >
                                <Workflow className="w-4 h-4 text-[#6941C6]" />
                                <div className={`text-sm font-semibold font-encode leading-5 flex items-center gap-1 ${!agentId ? 'text-[#344054]' : 'text-[#535862]'}`}>
                                    <span className='text-[#535862] font-medium text-xs mb-[1px]'>iFlow</span>
